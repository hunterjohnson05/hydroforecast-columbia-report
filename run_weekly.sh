...
if [ ! -f "$DB_FILE" ]; then
    ...
    echo "✓ Database backfilled."

    step "First-time setup: compute daily flow (daily_kaf column)"
...
step "2/10 · forecast_bar_chart.py (monthly + Apr-1-season bar charts)"
...
step "5/10 · apr_aug_forecast_evolution.py (Apr-Aug 28-day LTA % chart)"
cd "$SCRIPTS_DIR" && "$PYTHON" apr_aug_forecast_evolution.py --season apr-aug

step "6/10 · apr_aug_forecast_evolution.py (Apr-Sep 28-day LTA % chart)"
...
printf "\n\033[1;32mAll 10 steps done.\033[0m  Report: %s\n" "$REPORT"
open "$REPORT"
